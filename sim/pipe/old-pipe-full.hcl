# Jiaming Ji (jj35553)
# 
# Description of the pipe-full.hcl: 
# iaddq: 
...
#   # PC update:     PC <- valP
#
# The machine will need to check whether the instruction is valid, 
# whether it requres a register ID, since iaddq does require one register 
# for storing the final result and reading the value from the register 
# iaadq also read a constant value from the PC, the value that is added to the 
# register. 
# iaadq requires a source register to read value from, and that register is rB, 
# rB is also going to be used as the destination register to store the result. 
# In the ALU, we need to feed it with two inputs, one is valC and one is valB 
# we also want to set the condition code for iaddq 
